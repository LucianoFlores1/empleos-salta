...
export function inferCategory(title: string): string {
  ...
  if (t.match(/vent|comercial|cajer|atenci|vendedor|promotor|kiosc|mostrador|drugstore/)) return 'Ventas y Atención';
  ...
  if (t.match(/limpiez|mantenimient|mucam|maestranza|guardi|seguridad|bomba|operador|t[ée]cnico|mecanic|soldador|torner|sereno|vigilancia|instalador|tecno en redes|stock/)) return 'Oficios y Mantenimiento';
